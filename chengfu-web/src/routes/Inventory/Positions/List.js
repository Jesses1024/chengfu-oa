...
@connect(({ positionList, loading }) => ({
  positionList,
  loading: loading.models.positionList,
}))
@Form.create()
export default class PositionList extends PureComponent {
  ...
  gotoDetail = (id) => {
    this.props.dispatch(routerRedux.push({
      pathname: '/inventory/position-form',
      search: id ? `id=${id}` : '',
    }));
  }
  ...
  changeToggelDisable = (item) => {
    this.props.dispatch({
      type: 'positionList/toggleDisable',
      payload: {
        id: item.id,
        type: item.enabled,
      },
  ...
  render() {
    const { positionList: { data }, loading } = this.props;

    const columns = [{
      title: '操作',
      render: item => (
        <HasPermission perms={['inventory:position:edit']} noMatch="-">
          <Popconfirm title={`您确定要${item.enabled ? '停用' : '启用'}该仓位吗?`} onConfirm={() => this.changeToggelDisable(item)} okText="是" cancelText="否">
        ...
        <Card bordered={false}>
          <div className={styles.tableList}>
            ...
            <div className={styles.tableListOperator}>
              <HasPermission perms={['inventory:position:edit']}>
